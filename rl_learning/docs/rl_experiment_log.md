...
## v37: ball-state-free observation (sim2real prep)

Drop `ball_pos` (3) and `ball_vel` (3) from the env's observation
vector. New obs is 16-dim:
...
Motivation: ball state is sim-privileged. On the real Piper, ball
position and velocity cannot be reliably estimated at sub-throw
timescales (camera latency, occlusion by gripper, ball-too-fast for
kHz motion capture). A policy that depends on those slots would not
transfer. Strip them at training time so transfer is honest by
construction.

Cup_xy and pedestal_height stay in the obs — both are observable on
the real robot (perception or fiducials for cup_xy, known stack count
for pedestal_height) — and dropping them would make the task
ill-posed.

### Why a fresh run vs warm-start

22→16 obs change invalidates v36's policy first-layer (input dim
changes) and `vecnormalize.pkl` (running stats are 22-shaped).
Surgical first-layer reinit was considered and rejected — the
surviving deep layers had representations encoding the old obs
distribution. From-scratch is cleaner and matches the v34→v36
precedent (v36 was also from-scratch after v34→v35 warm-start
collapsed under semantic obs slot changes).

### Training config

Identical to v36 except for the env's observation space:

- 25M timesteps, from scratch.
- 16 envs SubprocVec, n_steps=2048, batch_size=512.
- LR=2e-4 linear, gamma=0.99, gae_lambda=0.95, ent_coef=0.01.
- Auto reward curriculum 1→2→3 (no R-stage / no Z-stage curriculum).
- R3 + Z3 from t=0.
- MlpPolicy medium 256×256, tanh.
- `release_step=45`, `action_delta=0.06`, `RESET_NOISE_STD=0.005`.
...
| Sustained range_success ≥ 0.5 (16-ep eval) | ~12M |
| Sustained range_success = 1.0 (16-ep eval) | ~17M |
| Final | 25M |

Stage promotions and early-training trajectory matched v36 closely —
removing ball state did not slow exploration, consistent with the
hypothesis that the dropped slots were redundant with joint state
during the welded windup.

### Final eval (deterministic seed=0, final 25M snapshot)

| metric | v36 (`v1`, 22-dim) | v37 (`no_ball_obs_v1`, 16-dim) |
|---|---:|---:|
| `range_success_rate` (300 ep R3 × Z3) | 0.807 | **0.940** |
| `exact_one_bounce_rate` (range eval) | — | 0.940 |
| `valid_bounce_rate` (range eval) | 1.000 | 1.000 |
| `grid3d_success_rate` (27 cells) | 0.741 | **0.963** |
| z=0 layer | 7/9 | 9/9 |
| z=7.5cm layer | 8/9 | 9/9 |
| z=15cm layer | 5/9 | 8/9 |
| `median_closest_cup_dist` (grid3d) | 0.019 m | 0.014 m |
| `success_rate` (8 ep fixed nominal) | 1.000 | 0.750 |
| `mean_reward` (grid3d) | 141.7 | 135.7 |

The 16-dim policy beats the 22-dim baseline on every randomized
metric. The single regressed metric is the 8-episode fixed-cup eval —
final-snapshot variance, late-training evals oscillated 0.750–1.000 on
that 8-sample test. The 300-episode random eval and the 27-cell grid
eval are both up substantially, so the model is generalizing better,
not regressing on the easy case in a structural way.

### Why the obs strip improved generalization

Hypothesis: ball state was effectively redundant during the windup
(the ball is welded to the gripper from `reset()` until step 45, so
`ball_pos` is a deterministic function of joint state) and
uninformative after release (the policy can no longer affect the
ball's flight). The dropped slots carried no information the policy
could act on, but the network still allocated capacity to learning
their representation. Removing them likely acted as mild
regularization — the network could not overfit to ball-state
correlations, which pushed it toward joint-state-only solutions that
generalize across the workspace box more uniformly.

This is preliminary — single seed, single comparison. Worth replicating
on a second seed before treating it as a real finding. But the
direction matches the underlying mechanics.

### Shipped artifacts

- `models/random_stack_cup_thrower_no_ball_obs_v1/` — final 25M
  snapshot + matching `vecnormalize.pkl`.
- Run dir (gitignored): `sim/_rl_out/no_ball_obs_v1_25m/`.
- Design plan: `docs/plans/2026-05-22-no-ball-obs-design.md`.

### Caveats / things to watch out for (additions)

- **Env obs space is now incompatible with all pre-v37 checkpoints.**
  v1, v2, `random_pos_*`, `single_bounce_*` all have 22-dim policies
  and 22-dim vecnormalize stats. Loading any of them against the
  current env throws an obs-shape mismatch in `PPO.load`. To play
  pre-v37 models, check out the parent commit (`8b27c01`) or its
  ancestors.

- **Single-seed result.** The improvement vs v1 is plausibly
  amplified by seed luck on top of the regularization effect. Worth
  confirming on a second seed before generalizing the conclusion.

- **8-episode fixed eval is too small to compare snapshots.** The
  variance on this metric across ~10 late-training evals was 0.250.
  Use the 300-episode range eval or the 27-cell grid as the headline
