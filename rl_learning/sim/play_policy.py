...
def _resolve_training_json(model_arg: str) -> Path:
    """Return the training.json that lives next to a model artifact.

    Mirrors ``_resolve_model_paths`` so a directory model resolves to
    ``<dir>/training.json`` and a stem/zip resolves to
    ``<stem>.training.json``.
    """
    p = Path(model_arg)
    if p.is_dir():
        return p / "training.json"
    if p.suffix == ".zip":
        return p.with_suffix(".training.json")
    return p.with_suffix(".training.json")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override per-step joint-target delta. Default: read from <model>/training.json, falling back to env constructor default (0.06).",
    )
    parser.add_argument(
        "--action-filter-alpha",
        type=float,
        default=None,
        help="Override action low-pass filter coefficient. Default: read from training.json, falling back to 1.0 (no filter).",
    )
    parser.add_argument(
        "--action-latency-range",
        type=str,
        default=None,
        help="Override action latency range as 'lo,hi' (per-episode sample). Default: read from training.json, falling back to '0,0' (no latency).",
    )
    parser.add_argument(
        "--obs-joint-pos-noise-std",
        type=float,
        default=None,
        help="Override obs joint_pos noise std. Default: read from training.json, falling back to 0.0.",
    )
    parser.add_argument(
        "--joint-pos-history-len",
        type=int,
        default=None,
        help="Override joint_pos history length. Default: read from training.json, falling back to 1.",
    )
    parser.add_argument(
        "--action-history-len",
        type=int,
        default=None,
        help="Override action history length. Default: read from training.json, falling back to 0.",
    )
    args = parser.parse_args()

    # Pull persisted env config from training.json so the playback env
    # matches what the model was trained against. CLI flags override
    # individual fields; missing file or fields fall back to RageCageEnv
    # constructor defaults (back-compat with pre-smooth_v1 models).
    env_kwargs = (
    ...
    if args.action_filter_alpha is not None:
        env_kwargs["action_filter_alpha"] = args.action_filter_alpha
    if args.action_latency_range is not None:
        parts = [p.strip() for p in args.action_latency_range.split(",")]
        if len(parts) != 2:
            parser.error(f"--action-latency-range must be 'lo,hi'; got {args.action_latency_range!r}")
        env_kwargs["action_latency_range"] = (int(parts[0]), int(parts[1]))
    if args.obs_joint_pos_noise_std is not None:
        env_kwargs["obs_joint_pos_noise_std"] = args.obs_joint_pos_noise_std
    if args.joint_pos_history_len is not None:
        env_kwargs["joint_pos_history_len"] = args.joint_pos_history_len
    if args.action_history_len is not None:
        env_kwargs["action_history_len"] = args.action_history_len
    if env_kwargs:
        print(f"env_kwargs (from training.json + overrides): {env_kwargs}")
    ...
    if args.model == "random":
        ...
        ppo = PPO.load(str(policy_zip), custom_objects=PPO_INFERENCE_CUSTOM_OBJECTS)
