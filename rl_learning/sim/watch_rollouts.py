...
from sim.train_rl import GRID_OFFSETS, RUN_ENV_KWARGS, render_policy_rollout_at_cup
...
def _load_model(model_path: Path, vec_path: Path, reward_stage: int) -> PPO:
    env = DummyVecEnv(
        [lambda: RageCageEnv(randomize_cup=False, reward_stage=reward_stage, **RUN_ENV_KWARGS)]
    )
...
def main() -> None:
    ...
    # Match the env config the model was trained against, so playback
    # uses the same action_delta / filter / latency. Older runs without
    # these fields fall through to constructor defaults (back-compat).
    persisted_env_kwargs = env_kwargs_from_training_json(args.run_dir / "training.json")
    if persisted_env_kwargs:
        RUN_ENV_KWARGS.update(persisted_env_kwargs)
        print(f"env_kwargs from {args.run_dir}/training.json: {persisted_env_kwargs}")
    if args.cells == "grid":
        cells = [
